...
print 'step01'
answers = []
f_text = open('reducecorpus/comment936_nopunction&nostopwords.txt')
for line in f_text.readlines():
    ...
    # # documents.append(document_sentence)
    # sentence=line[0].split()
    # for s in sentence:
    #     if s in stop_list():
    #         sentence.remove(s)
    # sentence=' '.join(sentence)
    # documents.append(sentence.encode('utf-8'))
    line=line.decode('utf-8')
    line = line.split(u'\u0001')
    answers.append(line[1].encode('utf-8'))
    # document_list = jieba.cut(line[0].encode('utf-8'))
...
corpora.MmCorpus.serialize('comment.mm', corpus)
